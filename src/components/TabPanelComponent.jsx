...
const TabPanelComponent = ({
  ...
  // const [createNewChild, setCreateNewChild] = useState([]);
    ...
    <>
      <ul className="flex items-center border-b border-gray-300">
        ...
        <li className="py-2 inline-flex items-center">
          <button
            className="p-1 rounded-full text-sm text-black focus:outline-none focus:shadow-outline focus:bg-red-600 hover:bg-red-600 hover:text-white transition duration-300 ease-in-out"
            onClick={() => {
              const newTab = `Room ${tabCounter++}`;
      ...
      <div className="p-4">
        {tabs.map((tab) => (
          <TabPanel key={tab} hidden={selectedTab !== tab}>
            Adults <button onClick={() => setNewAdult(newAdult - 1)}>-</button>
            {newAdult}{" "}
...
const TabSelector = ({ isActive, children, onClick, onClose }) => (
  <li className="relative">
    ...
    <span className="flex items-center absolute top-0 right-0 bottom-0">
      <button
        className="-ml-12 p-px rounded-full text-sm focus:outline-none focus:shadow-outline focus:bg-red-600 hover:bg-red-600 hover:text-white transition duration-300 ease-in-out"
        onClick={onClose}
        title="Close tab"
      >
        <svg
